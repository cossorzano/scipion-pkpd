...
class TestInhalation7Workflow(TestWorkflow):
    ...
    def testDissolutionWorkflow(self):
        if not os.path.exists('/home/coss/inh7.pkl'):
            # Lung parameters
            ...
            fhout = open('/home/coss/inh7.pkl','wb')
            pickle.dump([AUC_brtis, Ct_brtis, AUC_sys, Cmax_sys],fhout)
            fhout.close()
        else:
            fhIn = open('/home/coss/inh7.pkl','rb')
            [AUC_brtis, Ct_brtis, AUC_sys, Cmax_sys]=pickle.load(fhIn)
            fhIn.close()
